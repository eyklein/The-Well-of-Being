...
class AudioContent extends Content{
	...
	constructor(contentJson_,parentScene_){ //,url_, content_, propertiesJSON_
		...
		this.loop=false;//default false

		if(this.content.background){
			this.isBackgroundTrack=true;
		}else{
			this.isBackgroundTrack = false; //default track
		}
		...
		if(!this.isBackgroundTrack){
			this.gainNode.gain.value=currentStory.volume["main"]; //default
		}else{
			this.gainNode.gain.value=currentStory.volume["background"]; //default
		}
	...
	stop(){
		this.startTimestamp = context.currentTime-this.durationLeft;//this will force it to think it is done playing
		...
		// this.pause();
		// this.source.startingPosition=this.currentPlayTime;
	}
	...
	updateVolume(){
		if(this.isBackgroundTrack==false){
			this.gainNode.gain.value=currentStory.volume["main"];
		}else{
			this.gainNode.gain.value=currentStory.volume["background"];
		}
	}
	...
	getDurationLeft(){
		if(this.isPlaying==true){
			...
			return this.durationLeft-timeElapsed;
		}else{
			// let timeElapsed=context.currentTime-this.startTimestamp
	...
	getCurrentPlayTime(){//??
		if(this.isPlaying){
			let timeElapsed=context.currentTime-this.startTimestamp
			return this.currentPlayTime + timeElapsed;
		}else{
	...
	pause(){//this pauses just this audio clip
		if(this.isPlaying){
			...
			// this.durationLeft= this.getDurationLeft();
			// this.currentPlayTime = this.getCurrentPlayTime(); //this.currentPlayTime + timeElapsed;
		
			
			// this.isPlaying=false;//must be after duration left
		}else{
	...
	addToDraw(transcriptContent_){
		if(this.transcripts.indexOf(transcriptContent_)==-1){
			this.transcripts.push(transcriptContent_)
		}
	}
	updateTranscriptTimes(){
		for(let i in this.transcripts){
			this.transcripts[i].resetText();
		}
	}

	// addToContext() { //startPosition_,duration_
	...
	play(){
		...
		if(this.isPlaying){
			...
			// if(this.durationLeft<=0){//if finsihed playing restart
			// 	this.setInitalTimeVars();
			// }
		 	this.source.start(0,this.currentPlayTime,this.durationLeft);  
		 	// 0 or context.currentTime (0 means as soon as posible context.currentTime is prob better from an acounting piont of view)


		 	// console.log(this.id + " " + this.interval)
		 	if(this.isBackgroundTrack==false){
		 		console.log(this.id + " " + this.interval)
 			 	this.interval=setInterval(this.draw.bind(this),100); //10X a second is this enough
